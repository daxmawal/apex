...
constexpr const char* kKernelName = "replay_buffers_async_kernel";
constexpr int kSize = 256;
constexpr double kSentinel = -23.0;

using ReplayExecutionSpace = Kokkos::DefaultExecutionSpace;
using ReplayView = Kokkos::View<double*>;
...
bool matches_selected_input(const ReplayView& a, const ReplayView& b,
    ...
    for (int i = 0 ; i < kSize ; i++) {
        const double expected = (factor == 1) ? host_a(i) : host_b(i);
...
void run_device_work(const ReplayExecutionSpace& exec, ReplayView a, ReplayView b,
    ReplayView c, int factor) {
    if (factor == 1) {
        Kokkos::deep_copy(exec, c, a);
    } else {
        Kokkos::deep_copy(exec, c, b);
    }
}
...
int main(int argc, char* argv[]) {
    ...
    cache << "/tmp/apex_kokkos_replay_buffers_async_" << getpid()
          << ".yaml";
    ...
    {
        if (std::is_same<ReplayExecutionSpace,
                Kokkos::DefaultHostExecutionSpace>::value) {
            Kokkos::finalize();
            return 0;
        }

        ReplayExecutionSpace exec;
        ReplayView a("a_async", kSize);
        ReplayView b("b_async", kSize);
        ReplayView c_real("c_real_async", kSize);
        ReplayView a_replay("a_replay_async", kSize);
        ReplayView b_replay("b_replay_async", kSize);
        ReplayView c_replay("c_replay_async", kSize);
        ...
        for (int i = 0 ; i < kSize ; i++) {
            host_a(i) = static_cast<double>(i + 1);
            host_b(i) = static_cast<double>((i % 11) + 1);
        ...
        const size_t kernel_id = 301;
        const size_t factor_id = 401;
        kokkosp_declare_input_type("kokkos.kernel_name", kernel_id,
            kernel_info);
        kokkosp_declare_output_type("replay_buffers_async.factor", factor_id,
            factor_info);
        ...
        auto execute_tuned_context = [&](ReplayView in_a, ReplayView in_b,
            ...
            static size_t next_context = 3000;
            ...
            run_device_work(exec, in_a, in_b, out, factor);
            exec.fence();
        ...
        auto prepare = [&]() {
            prepare_calls++;
            Kokkos::deep_copy(exec, a_replay, a);
            Kokkos::deep_copy(exec, b_replay, b);
            Kokkos::deep_copy(exec, c_replay, 0.0);
            exec.fence();
        };
        auto replay_body = [&]() {
        ...
        if (!result.converged) {
            return fail("Async replay did not converge before max_replays.");
        }
        if (result.stopped_on_invalid || result.max_replays_reached) {
            return fail("Async replay stopped for the wrong reason.");
        }
        if (prepare_calls != result.replays || result.replays == 0) {
            return fail("Async replay did not call prepare once per replay.");
        }
        if (!matches_selected_input(a_replay, b_replay, c_replay, 1) &&
            !matches_selected_input(a_replay, b_replay, c_replay, 2)) {
            return fail("Async replay body did not update the replay output view.");
        }
        if (!all_equal(c_real, kSentinel)) {
            return fail("Async replay modified the real output buffer.");
        }

        const int real_factor = execute_tuned_context(a, b, c_real);
        if (!matches_selected_input(a, b, c_real, real_factor)) {
            return fail("Async final execution did not produce the expected output.");
        }
    }
    Kokkos::finalize();
    return 0;
