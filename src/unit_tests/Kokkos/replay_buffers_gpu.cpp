...
constexpr const char* kKernelName = "replay_buffers_gpu_kernel";
constexpr int kSize = 256;
constexpr double kSentinel = -29.0;
...
void run_gpu_kernel(const ReplayExecutionSpace& exec, ReplayView a,
    ReplayView b, ReplayView c, int factor) {
    const double factor_value = static_cast<double>(factor);
    Kokkos::parallel_for(kKernelName,
        Kokkos::RangePolicy<ReplayExecutionSpace>(exec, 0, kSize),
        KOKKOS_LAMBDA(const int i) {
            c(i) = a(i) + factor_value * b(i);
        });
}
...
int main(int argc, char* argv[]) {
    ...
    cache << "/tmp/apex_kokkos_replay_buffers_gpu_" << getpid()
          << ".yaml";
    ...
    Kokkos::initialize(argc, argv);
    {
        ...
        ReplayView a("a_gpu", kSize);
        ReplayView b("b_gpu", kSize);
        ReplayView c_real("c_real_gpu", kSize);
        ReplayView a_replay("a_replay_gpu", kSize);
        ReplayView b_replay("b_replay_gpu", kSize);
        ReplayView c_replay("c_replay_gpu", kSize);
        ...
        for (int i = 0 ; i < kSize ; i++) {
            host_a(i) = static_cast<double>(i + 3);
            host_b(i) = static_cast<double>(i % 13) + 0.25;
        ...
        const size_t kernel_id = 501;
        const size_t factor_id = 601;
        kokkosp_declare_input_type("kokkos.kernel_name", kernel_id,
            kernel_info);
        kokkosp_declare_output_type("replay_buffers_gpu.factor", factor_id,
            factor_info);
        ...
        auto execute_tuned_context = [&](ReplayView in_a, ReplayView in_b,
            ...
            static size_t next_context = 5000;
            ...
            run_gpu_kernel(exec, in_a, in_b, out, factor);
            exec.fence();
        ...
        if (!result.converged) {
            return fail("GPU replay did not converge before max_replays.");
        }
        if (result.stopped_on_invalid || result.max_replays_reached) {
            return fail("GPU replay stopped for the wrong reason.");
        }
        if (prepare_calls != result.replays || result.replays == 0) {
            return fail("GPU replay did not call prepare once per replay.");
        }
        if (!matches_kernel_result(a_replay, b_replay, c_replay, 1) &&
            !matches_kernel_result(a_replay, b_replay, c_replay, 2)) {
            return fail("GPU replay body did not update the replay output view.");
        }
        if (!all_equal(c_real, kSentinel)) {
            return fail("GPU replay modified the real output buffer.");
        }

        const int real_factor = execute_tuned_context(a, b, c_real);
        if (!matches_kernel_result(a, b, c_real, real_factor)) {
            return fail("GPU final execution did not produce the expected output.");
        }
    }
    Kokkos::finalize();
    return 0;
