...
#define FOREACH_APEX_OPTION(macro) \
    ...
    macro (APEX_TRACE_MARKER_EVENTS, use_marker_events, bool, true, "Enable/disable marker events (create, schedule, destroy).") \
    ...
    macro (APEX_KOKKOS_TUNING_CACHE_ONLY, use_kokkos_tuning_cache_only, bool, false, "Enable Kokkos tuning cache replay without autotuning missing contexts.") \
    macro (APEX_KOKKOS_TUNING_CACHE_ALLOW_BEST_SO_FAR, kokkos_tuning_cache_allow_best_so_far, bool, false, "Allow Kokkos cache-only replay to apply non-converged best-so-far results.") \
...
#define FOREACH_APEX_STRING_OPTION(macro) \
    ...
    macro (APEX_OPENCL_LIBRARY, opencl_library, char*, "libOpenCL.so", "Specify the name and/or path to the OpenCL library.")
